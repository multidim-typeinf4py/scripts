...
def cli_entrypoint(
    ...
    for project, subset in (pbar := tqdm.tqdm(test_set.items())):
        ...
        if not overwrite and not extended and dataset_io.full_location().exists():
            ...
            extended
            and dataset_io.full_location().exists()
            and not extended_dataset_io.full_location().exists()
        ):
            print(
        ...
        if not overwrite and extended and extended_dataset_io.full_location().exists():
            print(
                f"Skipping {project}; extended dataset already exists and no extension was requested!"
            )

        # print("Building parametric representation")
        # collection[ExtendedTypeCollectionSchema.parametric_anno] = collection[
        #    ExtendedTypeCollectionSchema.anno
        # ].progress_apply(lambda anno: extending.make_parametric(anno))

        # print("Simple or complex?")
        # collection[ExtendedTypeCollectionSchema.simple_or_complex] = collection[
        #    ExtendedTypeCollectionSchema.anno
        # ].progress_apply(
